 ...
 */
WechatMessage.prototype.sendKeFuMessage = function(token, type, content, touser, callback, count) {
  count = count || 0;
	...
	async.waterfall([
		function(cb) {
          ...
          if (e) console.log('err: ' + JSON.stringify(err));
          ...
          return cb('发送客服信息不成功');
			...
			if (err) {
        if (count <= 5) {
          return _this.sendKeFuMessage(token, type, content, touser, callback, count + 1); // retry at most 5 times
        }
        else {
          result = err; // return err msg as result
        }
      }
