...
WechatMessage.prototype.sendKeFuMessage = function(token, type, content, touser, cb) {
	requestify.post(
		...
		this.makeKeFuMessage(type, content, touser)
	).then(function(response) {
	    console.log('Response for posting kefu message: ' + response.body);
	    if (cb)
	    	cb(response);
	});
	return;
...
WechatMessage.prototype.forwardMessage = function(token, touser, cb) {
	requestify.post(
		'https://api.weixin.qq.com/cgi-bin/message/custom/send?access_token=' + token,
		this.msg2KeFuMsg(touser)
	).then(function(response) {
	    console.log('Response for posting forwarded message: ' + response.body);
	    if (cb)
	    	cb(response);
	});
	return;
