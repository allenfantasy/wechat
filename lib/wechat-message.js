...
WechatMessage.prototype.isSystemCommand = function() {
	// return this.isRegisterCommand() || this.isResetCommandFromUserName;();
 ...
 * @param content {Object} content of the message in JSON format,
 		should conform to the docs
 * @param touser {String} openid of the recipientS
 * @param cb {Function} callback
 */
WechatMessage.prototype.sendKeFuMessage = function(token, type, content, touser, cb) {
	requestify.post(
		'https://api.weixin.qq.com/cgi-bin/message/custom/send?access_token=' + token, this.makeKeFuMessage(type, content, touser))
		.then(cb);
	return;
}
...
WechatMessage.prototype.sendResponseMessage = function(req, res, type, content) {
	var result = new Js2Xml('xml', req.wechatMessage.makeResponseMessage(type, content));
    console.log(util.format("%j", req.wechatMessage.makeResponseMessage(type, content)));
	res.send(result.toString());
	return;
}

WechatMessage.prototype.forwardMessage = function(touser, cb) {
	requestify.post(
		'https://api.weixin.qq.com/cgi-bin/message/custom/send?access_token=' + token, this.msg2KeFuMsg(touser))
		.then(cb);
	return;
}
/************************* Helper functions *******************************/
